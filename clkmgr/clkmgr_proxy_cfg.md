...
initialize and manage the other time synchronization services on the system.  
...
how the **Clock Manager** proxy service initializes, interacts with, and monitors
time synchronization services within each time base.
...
must be specified within length [64 octets]. Note that a single UTF-8 character
...
- The `interfaceName` must be specified within length [64 octects].
...
- Default parameter: "/var/run/ptp4l" (follow **ptp4l**(8) default)

`domainNumber`
: - The PTP domain number.  
- Range: [0 .. 255]  
- Default parameter: 0 (follow **ptp4l**(8) default)
...
- Default parameter: "/var/run/chrony/chronyd.sock" (follow **chronyd**(8) default)

# EXAMPLES

Below are examples of JSON configuration files that refers to different scenarios.  

1. Single time base (single ptp4l instance)
    ...
    Therefore, the time synchronization data are provided solely by the
...
1. Single time base (single chrony instance)
...
1. Single time base (default value)

    In this scenario, the proxy service subscribes to one **ptp4l**(8) instance
    and one **chronyd**(8) instance, both are grouped under a single time base.
    Therefore, the time synchronization data are provided both the **ptp4l**(8)
    and **chronyd**(8) service. The JSON configuration file includes one
    **ptp4l**(8) and one **chronyd**(8) object within same time base. As the
...
1. Single time base (with assigned values)
    ...
    Therefore, the time synchronization data are provided both the **ptp4l**(8)
    and **chronyd**(8) service. The JSON configuration file includes one
    **ptp4l**(8) and one **chronyd**(8) object within same time base.  
...
1. Multiple time bases
    ...
    are provided by the **ptp4l**(8) and **chronyd**(8) service in respective
