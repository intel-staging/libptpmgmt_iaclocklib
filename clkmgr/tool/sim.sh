...
make_clkmgr()
{
 ...
 if ! [[ -e clkmgr/proxy/clkmgr_proxy ]]; then
   make CXXFLAGS="$CXXFLAGS -U_FORTIFY_SOURCE" -C $base --no-print-directory
 fi
 a_patch+=:clkmgr/proxy
 if ! [[ -e clkmgr/sample/clkmgr_test ]]; then
   make -C $base/clkmgr/sample --no-print-directory
 fi
