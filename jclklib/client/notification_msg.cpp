...
PROCESS_MESSAGE_TYPE(ClientNotificationMessage::processMessage)
{
    ...
    for (it=ClientStateArray.begin(); it<ClientStateArray.end();it++)
    {
        ...
        currentClientState->get_eventSub().get_composite_event().readEvent(composite_eventSub,
        ...
        if ((eventSub[0] & 1<<gmChangedEvent) &&
            ...
            client_ptp_data->gm_changed_event_count.fetch_add(1, std::memory_order_relaxed);
