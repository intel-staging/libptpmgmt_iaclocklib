/** @file transport.hpp
 * @brief Common transport base class for JClkLib. It is extended for specific transports such as POSIX message queue.
 *
...
#define LISTENER_CONTEXT_PROCESS_MESSAGE_TYPE(name)			\
		bool name(JClkLibCommon::Message *bmsg, JClkLibCommon::TransportTransmitterContext *&txcontext)
...
#define CREATE_TRANSMIT_CONTEXT_TYPE(name)					\
		JClkLibCommon::TransportTransmitterContext *name(JClkLibCommon::TransportClientId &clientId)
	...
	public:
		...
		typedef std::function<bool(TransportWorkArg::element_type *)> TransportWorkFunction;
		typedef std::pair<TransportWorkFunction,TransportWorkArg> TransportWork;
		typedef ptrdiff_t TransportWorkDesc;
		static const TransportWorkDesc InvalidTransportWorkDesc = (TransportWorkDesc) -1;
	private:
		...
		static void dispatchLoop(std::promise<FUTURE_TYPEOF(TransportWorkerState::retVal)>,
