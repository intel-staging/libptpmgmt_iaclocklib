...
#define PARSE_RX(type,var,lc)						\
	({								\
	decltype(lc.getc_offset()) offset = PARSE_##type(var,lc.getc_buffer().data() + lc.getc_offset(), \
...
#define PARSE_FIELD(var,ptr,len) _PARSE(std::remove_reference<decltype(var)>::type,var,ptr,len)
...
({									\
	decltype(tc.getc_offset()) offset = WRITE_##type(var,tc.get_buffer().data() + tc.get_offset(), \
...
#define WRITE_FIELD(var,ptr,len) _WRITE(std::remove_reference<decltype(var)>::type,var,ptr,len)
